...
5. **Generates appropriate TwiML**:
   - If Vapi is configured: Returns TwiML with `<Connect><Stream>` to connect to Vapi
   - If Vapi is not configured: Returns simple greeting (fallback mode)

### TwiML Structure

The generated TwiML looks like this:

```xml
<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Say voice="alice">Thank you for calling Salon Name. Please hold while we connect you to our AI assistant.</Say>
  <Connect>
    <Stream url="wss://api.vapi.ai">
      <Parameter name="assistantId" value="your-assistant-id" />
      <Parameter name="apiKey" value="your-api-key" />
      <Parameter name="metadata" value='{"tenantId":"...","tenantName":"...","callLogId":"..."}' />
    </Stream>
  </Connect>
</Response>
```
...
- **`<Say>`**: Plays a brief message while connection is established
- **`<Connect>`**: Connects the call to an external service
- **`<Stream>`**: Establishes WebSocket connection to Vapi
- **Parameters**: Pass configuration and metadata to Vapi
